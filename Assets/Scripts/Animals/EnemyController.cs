...
public class EnemyController : MonoBehaviour
{
    ...
    public Transform enemyTransform; // this field is so that the transform can be shared with the player
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position, movePoint.position) <= 0.5f && gameManager.state is GameState.ENEMYTURN)
        {
            MoveEnemy();
        }
    }
